...
OUTPUT = "HHBKTendo_Sprechernotizen.pdf"
...
slide_num_style = ParagraphStyle("SlideNum", fontName="Helvetica", fontSize=8,
    textColor=PURPLE, alignment=TA_CENTER, spaceAfter=1)

title_style = ParagraphStyle("Title", fontName="Helvetica-Bold", fontSize=14,
    textColor=PINK, spaceAfter=3, leading=17)

subtitle_style = ParagraphStyle("Subtitle", fontName="Helvetica-Oblique", fontSize=9,
    textColor=CYAN, spaceAfter=5, leading=12)

body_style = ParagraphStyle("Body", fontName="Helvetica", fontSize=10,
    textColor=WHITE, spaceAfter=6, leading=15, alignment=TA_JUSTIFY)

section_style = ParagraphStyle("Section", fontName="Helvetica-Bold", fontSize=10,
    textColor=CYAN, spaceAfter=3, spaceBefore=7, leading=13)

bullet_style = ParagraphStyle("Bullet", fontName="Helvetica", fontSize=10,
    textColor=WHITE, leftIndent=12, spaceAfter=4, leading=14)

tip_style = ParagraphStyle("Tip", fontName="Helvetica-Bold", fontSize=9,
    textColor=YELLOW, leftIndent=0, spaceAfter=4, leading=13,
    borderPad=4)

code_style = ParagraphStyle("Code", fontName="Courier", fontSize=8,
    textColor=CYAN, leftIndent=10, spaceAfter=4, leading=12, backColor=colors.HexColor("#0A0A20"))

def T(text): return Paragraph(text, title_style)
def ST(text): return Paragraph(text, subtitle_style)
...
def header(num, title, subtitle=""):
    items = [Paragraph(f"Folie {num}", slide_num_style), SP(1), T(title)]
    if subtitle: items.append(ST(subtitle))
    items.append(HR())
    return items
...
# FOLIE 1 – Titelfolie
# ════════════════════════════════════════════════════════════════════════════
story += header(1, "HHBKTendo – Spielesammlung", "Eröffnung")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(2, "Projektüberblick")
story += [
    P("Das Projekt hat vier Kernbereiche:"),
    B("<b>Spielesammlung:</b> Zwei vollständige Strategiespiele auf einem 6×6-Feld. Beide Spiele sind komplett spielbar gegen eine KI."),
    B("<b>KI-Gegner:</b> Ein einziger, wiederverwendbarer MiniMax-Algorithmus mit Alpha-Beta-Pruning funktioniert für beide Spiele – kein duplizierter Code."),
    B("<b>Benutzerverwaltung:</b> Spieler können sich registrieren und einloggen. Passwörter werden mit SHA-256 und Salt gesichert. Alternativ ist auch ein Gast-Modus ohne Konto möglich."),
    B("<b>Bestenliste:</b> Alle Spielergebnisse werden in einer SQLite-Datenbank gespeichert und können nach Spiel und Schwierigkeitsgrad sortiert abgerufen werden."),
...
# ════════════════════════════════════════════════════════════════════════════
story += header(3, "Projektstruktur", "Modularer Aufbau")
story += [
    ...
    B("<b>pawn_chess.py / tictactoe.py</b> – Die eigentliche Spiellogik: erlaubte Züge berechnen, Züge anwenden, Gewinner ermitteln, Stellungen bewerten."),
...
# ════════════════════════════════════════════════════════════════════════════
# FOLIE 4 – Benutzerverwaltung & Datenbank
# ════════════════════════════════════════════════════════════════════════════
story += header(4, "Benutzerverwaltung & Datenbank", "auth.py · database.py")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(5, "Datenbank-Techniken", "row_factory · Migration · SQL-Injection-Schutz")
story += [
    P("Wir nutzen drei wichtige Techniken für eine robuste Datenbankanbindung:"),
    SEC("sqlite3.Row als row_factory"),
    P("Statt Spalten per Index anzusprechen (row[1]) kann man sie per Name abrufen: row['username']. Das macht den Code lesbarer und weniger fehleranfällig wenn sich die Spaltenreihenfolge ändert. Mit dict(row) lässt sich das Ergebnis einfach in ein Dictionary umwandeln."),
    ...
    P("Wenn eine neue Spalte zur DB hinzugefügt wird, wird der ALTER TABLE-Befehl in einem try/except-Block ausgeführt. Falls die Spalte schon existiert, wird der Fehler ignoriert – so bleibt die DB abwärtskompatibel."),
    TIP("Gutes Beispiel für defensives Programmieren – kurz den Sicherheitsaspekt betonen."),
    PageBreak(),
]

# ════════════════════════════════════════════════════════════════════════════
# FOLIE 6 – Passwortsicherheit
# ════════════════════════════════════════════════════════════════════════════
story += header(6, "Sichere Passwortspeicherung", "SHA-256 + Salt")
story += [
    ...
    B("<b>os.urandom(16)</b> erzeugt 16 kryptografisch zufällige Bytes als Salt. Das ist wichtig, weil ein zufälliger Salt dafür sorgt, dass zwei gleiche Passwörter unterschiedliche Hashes erzeugen."),
...
# FOLIE 7 – MiniMax
# ════════════════════════════════════════════════════════════════════════════
story += header(7, "MiniMax-Algorithmus", "mit Alpha-Beta-Pruning")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(8, "Callback-basiertes MiniMax-Design", "Eine KI-Engine für alle Spiele")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(9, "Bauernschach", "6×6 · Weiß (Mensch) vs. Schwarz (KI)")
story += [
    ...
    B("Figurenzahl + Vorwärtsfortschritt + Zentrumskontrolle."),
...
# ════════════════════════════════════════════════════════════════════════════
story += header(10, "Bewertungsfunktion Bauernschach", "Heuristik: Figurenzahl · Fortschritt · Zentrum")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(11, "Tic-Tac-Toe (4 gewinnt)", "6×6 · Mensch (X) vs. KI (O)")
story += [
    ...
    B("<b>Kandidatenzüge:</b> Statt alle 36 leeren Felder zu prüfen, betrachtet die KI nur Felder, die an bereits besetzte Felder angrenzen. Das reduziert die Züge auf 8–15."),
...
# ════════════════════════════════════════════════════════════════════════════
# FOLIE 12 – TTT Optimierungen
# ════════════════════════════════════════════════════════════════════════════
story += header(12, "TTT Optimierungen im Detail", "Kandidatenzüge · Center-Sorting · shallow copy")
story += [
    SEC("Kandidatenzüge"),
    P("In einem 6×6-Feld gibt es anfangs 36 mögliche Züge. Die KI schränkt das auf Felder ein, die direkt neben einem bereits besetzten Feld liegen. Das ist sinnvoll, weil ein Stein in der Mitte des Feldes ohne Nachbarn strategisch fast wertlos ist. Dadurch sinkt die Anzahl der zu prüfenden Züge auf typischerweise 8–15."),
    SEC("Center-Sorting"),
    P("Kandidaten werden vor der Auswertung nach ihrer Entfernung zum Mittelpunkt sortiert. Zentrumsnähe ist ein gutes Qualitätsmerkmal für Züge. Da Alpha-Beta bessere Züge zuerst sehen und mehr Äste abschneiden kann, beschleunigt das die Suche erheblich."),
    SEC("shallow copy statt deepcopy"),
    P("Das Spielfeld wird als Liste von Listen kopiert: [row[:] for row in board]. Das ist etwa 5× schneller als deepcopy(board), weil nur die Zeilenlisten flach kopiert werden – ausreichend für ein 6×6-Feld mit ganzen Zahlen."),
...
# ════════════════════════════════════════════════════════════════════════════
story += header(13, "GUI & Threading", "daemon Thread · after(0, ...) · app_state")
story += [
...
# FOLIE 14 – Unittests
# ════════════════════════════════════════════════════════════════════════════
story += header(14, "Unittests", "TC-01 bis TC-15 · test_all.py · pytest-kompatibel")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
# FOLIE 15 – Test-Infrastruktur
# ════════════════════════════════════════════════════════════════════════════
story += header(15, "Test-Infrastruktur", "unittest · Temp-DB · Mocking")
story += [
    ...
    SEC("patch.object"),
    P("Mit unittest.mock.patch.object wird database.DB_PATH auf den temporären Pfad umgeleitet. Das bedeutet: der getestete Code läuft unverändert, schreibt aber in die Temp-DB statt in die echte games.db."),
...
# ════════════════════════════════════════════════════════════════════════════
story += header(16, "Zusammenfassung", "Was haben wir erreicht?")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(17, "SOLL/IST Feature-Vergleich", "Lastenheft → Pflichtenheft → Umsetzung → Test")
story += [
...
# ════════════════════════════════════════════════════════════════════════════
story += header(18, "SOLL/IST Projektplanung", "Wasserfallmodell · 2 Meetings · Discord")
story += [
...
# FOLIE 19 – GUI
# ════════════════════════════════════════════════════════════════════════════
story += header(19, "Benutzeroberfläche (GUI)", "tkinter · HHBKTendo Design System · Dark Theme")
story += [
    SEC("Screen-Flow"),
...
# FOLIE 20 – Quellen
# ════════════════════════════════════════════════════════════════════════════
story += header(20, "Quellen & Werkzeuge")
story += [
...
# FOLIE 21 – Fazit
# ════════════════════════════════════════════════════════════════════════════
story += header(21, "Fazit – Projektverlauf", "Team · Highlights · Lessons Learnt")
story += [
    ...
    B("Schnittstellen zwischen Modulen (z.B. Callback-Signaturen) detaillierter im Pflichtenheft definieren."),
    SEC("Ausblick"),
